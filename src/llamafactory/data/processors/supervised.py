...
def _encode_supervised_example(
    ...
    messages = template.mm_plugin.process_messages(
        prompt + response, images, videos, processor
    )
    ...
    logger.debug(
        f"{len(messages) = }, {total_length = }, {len(input_ids) = }, {len(labels) = }, {labels[:10] = }"
    )
    if mask_history:
...
def preprocess_packed_supervised_dataset(
    ...
    for i in range(len(examples["_prompt"])):
        ...
        if length > data_args.cutoff_len:
            logger.warning_rank0(
                f"Dropped lengthy example with length {length} > {data_args.cutoff_len}."
            )
        else:
    ...
    knapsacks = greedy_knapsack(
        lengths, data_args.cutoff_len - 1
